...
class DailyRecordSchema(Schema):
    ...
    bulk_ente = fields.Float(required=True, validate=lambda x: x >= 0.0)
    bulk_prod = fields.Float(required=True, validate=lambda x: x >= 0.0)
    bulk_inve = fields.Float(required=True, validate=lambda x: x >= 0.0)
